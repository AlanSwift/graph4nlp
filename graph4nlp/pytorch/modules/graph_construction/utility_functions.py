...
class Node:
    ...
    sentence_ : (int)
        To illustrate which sentence the node is related when multiple sentences \
            included in a graph.
    """
...
class UtilityFunctionsForGraph:
    ...
    def get_head_node(g, sentence_id):
        """Return the head node in a syntactic graph, to be used when we need to connect \
            multiple graphs into one merged graph."""
        for n in g.nodes():
    ...
    def get_tail_node(g, sentence_id):
        """Return the tail node in a syntactic graph, to be used when we need to connect \
            multiple graphs into one merged graph."""
        for n in g.nodes():
